...
int main() {
	...
	for (std::size_t i = 0; i < quan.packages; ++i) {
		...
		case iware::cpu::endianness_t::big:
			std::cout << "Big-Endian";
			break;
		case iware::cpu::endianness_t::middle:
		...
		for (int i = 1; i < 4; ++i) {
			...
			std::cout << std::boolalpha << "\t" << "L" << i << " cache: size " << cache.size << ", line size: " << cache.line_size
	...
	for (std::size_t i = 0; i < devices.size(); ++i) {
		...
		std::cout << "\n"
			<< "\t"
			<< device.memory_size << " bytes, " 
			<< device.cache_size << " bytes cache"
			<< "\n";
	}
	std::cout << '\n';
